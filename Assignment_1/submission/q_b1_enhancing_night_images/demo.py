...
def main(args):
    ...
    for i, image in tqdm(enumerate(images), desc="Enhancing images"):
        ...
        g = 0.6
        l = 0
        enhanced_image = enhance_image_exposure(image, g, l, not args.lime,sigma=args.sigma, bc=args.bc, bs=args.bs, be=args.be, eps=args.eps)
        filename = basename(files[i])
        name, ext = splitext(filename)
        method = "LIME" if args.lime else "DUAL"
        #corrected_name = f"{name}{ext}"
        corrected_name = f"{name}_g={g}_l={l}{ext}"
