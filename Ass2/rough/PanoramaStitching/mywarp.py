...
if __name__ == '__main__':
    ...
    IMG_DIR = '../data/{}'.format(ex_no)
    N = len(os.listdir(IMG_DIR))                             # Number of Images to be matched
    Images = []
    ...
    print(f"||Setting the Reference image as {N//2}.||")
    ...
    left_H = []
    right_H = []
    cv2.imwrite("registration_output/img{}.jpg".format(0), img.astype('float'))
    cv2.imwrite("registration_output/mask{}.jpg".format(0), mask.astype('float'))
    for i in range(1, len(Images)//2+1):
        ...
        try:
            ...
            poc = matchfeatures(Images[N//2-i], Images[N//2-(i-1)], verbose=True)
            ...
            img_outputs.append(img)
            masks.append(mask)
            cv2.imwrite("registration_output/img{}.jpg".format(N//2-i), img)
            cv2.imshow("mask",mask)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
            cv2.imwrite("registration_output/mask{}.jpg".format(N//2-i), mask.astype('float'))
        except:
            pass
    ...
    cv2.imwrite("output/Panaroma_Image_{}.jpg".format(ex_no), final)
    print("Succesfully Saved image as Panaroma_Image_{}.jpg".format(ex_no))
